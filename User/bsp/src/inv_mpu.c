...
#define get_ms      bsp_GetMs

//INT引脚所要用的功能初始化
static int reg_int_cb(struct int_param_s *int_param)
{
	  struct int_param_s reg_int_param;
    reg_int_param = *int_param;
    reg_int_param.init_isr();
    return 0;
}
...
static int setup_compass(void)
{
    ...
    st.chip_cfg.mag_sens_adj[2] = (long)data[2] + 128;
		...
		mag_sens_adj_val[0] = st.chip_cfg.mag_sens_adj[0];
		mag_sens_adj_val[1] = st.chip_cfg.mag_sens_adj[1];
		mag_sens_adj_val[2] = st.chip_cfg.mag_sens_adj[2];
