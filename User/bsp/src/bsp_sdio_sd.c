...
*	文件名称 : bsp_sdio_sd.c
*	版    本 : V1.1
*	说    明 : 根据ST的资料整合并改编。原始文件为: stm3210e_eval_sdio_sd.c
*
*	修改记录 :
*		版本号  日期        作者     说明
*		V1.0    2015-08-02 armfly  正式发布
*		V1.1    2015-08-06 armfly  修改数据超时的循环次数。没有卡插入时，可以快速返回
...
static void SDIO_Interrupts_Config(void);
...
SD_Error SD_Init(void)
{
  ...
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;  /* 暂时配置成1bit模式 */	
...
static void SDIO_Interrupts_Config(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = SD_SDIO_PRE_PRIORITY;
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = SD_SDIO_SUB_PRIORITY;
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
  NVIC_Init(&NVIC_InitStructure);

  NVIC_InitStructure.NVIC_IRQChannel = SD_SDIO_DMA_IRQn;
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = SD_DMA2_PRE_PRIORITY;
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = SD_DMA2_SUB_PRIORITY;
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
  NVIC_Init(&NVIC_InitStructure);
}
