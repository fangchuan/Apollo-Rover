...
#define  GPS_PORT_RCC     RCC_APB2Periph_GPIOA
#define  GPS_UART_RCC			RCC_APB1Periph_USART2
...
void bsp_InitGPS(void)
{
	...
	RCC_APB2PeriphClockCmd(GPS_PORT_RCC | RCC_APB2Periph_AFIO, ENABLE);
	...
	/* 第4步： 配置串口硬件参数 */
	USART_InitStructure.USART_BaudRate = GPS_UART_BAUD;	/* 波特率 */
	...
	USART_ITConfig(GPS_UART, USART_IT_RXNE, ENABLE);	/* 使能接收中断 */
	USART_Cmd(GPS_UART, ENABLE);		/* 使能串口 */
	USART_ClearFlag(GPS_UART, USART_FLAG_TC);     /* 清发送完成标志，Transmission Complete flag */
#endif
...
//		}
//	else 
//		if (memcmp(_ucaBuf, "GPVTG,", 6) == 0)
//		{
//			gpsGPVTG(_ucaBuf, _usLen);
//		}
//	else 
//		if (memcmp(_ucaBuf, "GPGLL,", 6) == 0)
//		{
...
uint16_t gps_FenToMiao(uint32_t _fen)
{
	...
	/* g_tGPS.WeiDu_Fen;	纬度，分. 23.2475；  小数点后4位  表示 23.2475分 
